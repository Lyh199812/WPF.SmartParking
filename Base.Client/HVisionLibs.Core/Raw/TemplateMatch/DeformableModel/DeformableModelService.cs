...
namespace HVisionLibs.Core.TemplateMatch.DeformableModel
{
    public class DeformableModelService: BindableBase, ITemplateMatchService
    {
        ...
        public async Task CreateTemplate(HObject image, HObject hObject)
        {
            try
            {
                ...
                OpenFileDialog saveFileDialog = new OpenFileDialog();
                    saveFileDialog.Filter = "Deformable Model Files (*.dm)|*.dm|All Files (*.*)|*.*";
                saveFileDialog.Title = "Save Deformable Model";

                if (saveFileDialog.ShowDialog()!=null&& (bool)saveFileDialog.ShowDialog())
                {
                    modelFilePath= saveFileDialog.FileName;
                }
                else
                {
                    return;
                ...
                await Task.Run(() =>
                {
                    ...
                    // Save the deformable model to a file
                    HOperatorSet.WriteDeformableModel(modelId, modelFilePath);
                });
            }
            catch (Exception)
            {
        ...
        public MatchResult Run(HObject image)
        {
            ...
            {
                matchResult.Message = $"匹配失败,"+ex.ToString();
                return matchResult;
            }
