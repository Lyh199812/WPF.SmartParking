...
namespace Device.Communication
{
    public class ModbusTCP
    {
        ...
        public bool PreSetMultiRegisters(ushort start, byte[] values)
        {
            ...
            // 调整字节数据的顺序为cdab
            byte[] cdadValues = new byte[values.Length];
            for (int i = 0; i < values.Length; i += 4)
            {
                cdadValues[i] = values[i + 2];
                cdadValues[i + 1] = values[i + 3];
                cdadValues[i + 2] = values[i];
                cdadValues[i + 3] = values[i + 1];
            }

            //字节数据
            // SendCommand.Add(values);
            SendCommand.Add(cdadValues);
